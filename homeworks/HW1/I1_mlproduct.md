...
Please fork the albumy repository that contains the existing *albumy* implementation (you can change or replace all code in this repository however you like). You can optionally use [moments](https://github.com/greyli/moments) which is more recent version of albumy. 
...
Additionally upload a short report to Gradescope by the deadline with the following content: 

* **GitHub link:** Please upload a zip file of completed project. 
...
* 10p: The document is clearly structured, such that it is clear which text belongs to which question. The document includes a link *to a specific commit* in your GitHub repository. When uploading the solution to Gradescope, questions are mapped to the relevant pages.
